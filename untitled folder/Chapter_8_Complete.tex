...
│  ├─ Output: Position-level representations (1.2M × 512)        │
│                                                                  │
│  Task-Specific Heads                                            │
...
│                                                                  │
│  On-Target Efficiency: [ê - q_α, ê + q_α]                      │
...
│                                                                  │
│  Off-Target Risk: [p̂ - q'_α, p̂ + q'_α]                        │
...
│                                                                  │
│  Quality Score: e_on - λ·p_off,upper                            │
...
\hline
On-Target Prediction & 3 & Sequence+Epigenomics → Efficiency & $\hat{e} \in [0,1]$ \\
\hline
...
CRISPRO-MAMBA-X automatically retrieves required supporting data:
...
\begin{enumerate}
    ...
    \begin{equation}
    \mathbf{u}_k = [\text{RNA-FM embedding}_k; \text{ATAC}_k; H3K27ac_k; \text{Nucleosome}_k; \text{Methylation}_k; \text{Hi-C}_k] \in \mathbb{R}^{512+6}
    \end{equation}
...
\hline
Total training time & 30 epochs × 1.5 h = 45 hours & Including validation, ~2 days \\
\hline
GPU recommendation & 3× A100 (80 GB each) & Or 6× A6000 (48 GB) \\
\hline
\end{tabular}
\end{table}
...
│  • Compute prediction intervals: [ê ± q_α]                │
