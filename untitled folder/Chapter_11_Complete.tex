...
\begin{enumerate}
    ...
    \item \textbf{Wall-Clock Time:} 50 architectures × 10 hours / 4 GPUs (parallel evaluation) = 125 GPU-hours wall-clock ≈ 2-3 weeks
\end{enumerate}
...
\centering
\begin{verbatim}
Pareto Frontier: Accuracy vs Inference Latency
...
(Spearman)
    ...
    0.970 |      Mamba
          |        (0.970)
          |
    0.965 |    Sparse Transformer
          |       (0.968)     ViT (0.964)
          |
    0.960 |
          |                    GNN (0.958)
          |
    0.955 |         Epigenomics-only
          |              (0.954)
          |
    0.950 +--+--+--+--+--+--+--+--+--+--+
...
* Pareto optimal (no other architecture
  achieves better accuracy with lower latency)
\end{verbatim}
\end{figure}
