...
#SBATCH --output=results/caduceus_splitC_seed123/slurm-%j.out
#SBATCH --error=results/caduceus_splitC_seed123/slurm-%j.err

# ChromaGuide: caduceus | Split C | Seed 123
...
mkdir -p results/caduceus_splitC_seed123
...
if [ -f ~/scratch/chromaguide_v2/results/caduceus_splitC_seed123/metrics.json ]; then
    python -c "
import json
with open('~/scratch/chromaguide_v2/results/caduceus_splitC_seed123/metrics.json') as f:
    r = json.load(f)
