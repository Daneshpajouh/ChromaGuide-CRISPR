...
python experiments/train_experiment.py \
    ...
    --split B \
    --seed 123 \
    --output_dir results/cnn_gru_splitB_seed123
...
if [ -f results/cnn_gru_splitB_seed123/metrics.json ]; then
    python -c "
import json
with open('results/cnn_gru_splitB_seed123/metrics.json') as f:
    r = json.load(f)
