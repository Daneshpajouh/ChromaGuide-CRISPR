#!/bin/bash
#SBATCH --job-name=cg-cnn_gr-B-s123
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=4:00:00
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/cnn_gru_splitB_seed123/slurm-%j.out
#SBATCH --error=results/cnn_gru_splitB_seed123/slurm-%j.err

# ChromaGuide: cnn_gru | Split B | Seed 123
...
mkdir -p results/cnn_gru_splitB_seed123
...
if [ -f ~/scratch/chromaguide_v2/results/cnn_gru_splitB_seed123/metrics.json ]; then
    python -c "
import json
with open('~/scratch/chromaguide_v2/results/cnn_gru_splitB_seed123/metrics.json') as f:
    r = json.load(f)
