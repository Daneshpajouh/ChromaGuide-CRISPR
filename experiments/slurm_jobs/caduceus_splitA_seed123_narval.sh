#!/bin/bash
#SBATCH --job-name=cg-caduce-A-s123
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=6:00:00
#SBATCH --mem=64G
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/caduceus_splitA_seed123/slurm-%j.out
#SBATCH --error=results/caduceus_splitA_seed123/slurm-%j.err

# ChromaGuide: caduceus | Split A | Seed 123
...
mkdir -p results/caduceus_splitA_seed123
...
if [ -f ~/scratch/chromaguide_v2/results/caduceus_splitA_seed123/metrics.json ]; then
    python -c "
import json
with open('~/scratch/chromaguide_v2/results/caduceus_splitA_seed123/metrics.json') as f:
    r = json.load(f)
