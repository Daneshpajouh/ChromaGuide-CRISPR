#!/bin/bash
#SBATCH --job-name=cg-caduce-C-s42
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=6:00:00
#SBATCH --mem=64G
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/caduceus_splitC_seed42/slurm-%j.out
#SBATCH --error=results/caduceus_splitC_seed42/slurm-%j.err

# ============================================================
# ChromaGuide: caduceus | Split C | Seed 42
# Cluster: narval (A100)
# ============================================================
...
mkdir -p results/caduceus_splitC_seed42
...
if [ -f results/caduceus_splitC_seed42/results.json ]; then
    echo ""
    echo "=== RESULTS ==="
    python -c "
import json
with open('results/caduceus_splitC_seed42/results.json') as f:
    r = json.load(f)
