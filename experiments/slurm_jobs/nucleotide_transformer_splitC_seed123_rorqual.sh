#!/bin/bash
#SBATCH --job-name=cg-nucleo-C-s123
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=18:00:00
#SBATCH --mem=64G
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/nucleotide_transformer_splitC_seed123/slurm-%j.out
#SBATCH --error=results/nucleotide_transformer_splitC_seed123/slurm-%j.err

# ============================================================
# ChromaGuide: nucleotide_transformer | Split C | Seed 123
...
mkdir -p results/nucleotide_transformer_splitC_seed123
...
if [ -f results/nucleotide_transformer_splitC_seed123/results.json ]; then
    echo ""
    echo "=== RESULTS ==="
    python -c "
import json
with open('results/nucleotide_transformer_splitC_seed123/results.json') as f:
    r = json.load(f)
