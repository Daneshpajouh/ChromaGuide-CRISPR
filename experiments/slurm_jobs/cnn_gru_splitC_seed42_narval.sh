#!/bin/bash
#SBATCH --job-name=cg-cnn_gr-C-s42
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=4:00:00
#SBATCH --mem=64G
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/cnn_gru_splitC_seed42/slurm-%j.out
#SBATCH --error=results/cnn_gru_splitC_seed42/slurm-%j.err

# ============================================================
# ChromaGuide: cnn_gru | Split C | Seed 42
# Cluster: narval (A100)
# ============================================================
...
mkdir -p results/cnn_gru_splitC_seed42
...
if [ -f results/cnn_gru_splitC_seed42/results.json ]; then
    echo ""
    echo "=== RESULTS ==="
    python -c "
import json
with open('results/cnn_gru_splitC_seed42/results.json') as f:
    r = json.load(f)
