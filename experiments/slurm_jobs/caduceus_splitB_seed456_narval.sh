...
python experiments/train_experiment.py \
    ...
    --split B \
    --seed 456 \
    --output_dir results/caduceus_splitB_seed456
...
if [ -f results/caduceus_splitB_seed456/metrics.json ]; then
    python -c "
import json
with open('results/caduceus_splitB_seed456/metrics.json') as f:
    r = json.load(f)
