#!/bin/bash
#SBATCH --job-name=cg-cnn_gr-C-s456
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=4:00:00
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/cnn_gru_splitC_seed456/slurm-%j.out
#SBATCH --error=results/cnn_gru_splitC_seed456/slurm-%j.err

# ============================================================
# ChromaGuide: cnn_gru | Split C | Seed 456
# Cluster: nibi (GPU)
# ============================================================
...
mkdir -p results/cnn_gru_splitC_seed456
...
if [ -f results/cnn_gru_splitC_seed456/results.json ]; then
    echo ""
    echo "=== RESULTS ==="
    python -c "
import json
with open('results/cnn_gru_splitC_seed456/results.json') as f:
    r = json.load(f)
