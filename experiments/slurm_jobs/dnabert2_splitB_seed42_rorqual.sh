#!/bin/bash
#SBATCH --job-name=cg-dnaber-B-s42
#SBATCH --account=def-kwiese_gpu
#SBATCH --time=12:00:00
#SBATCH --mem=64G
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --output=results/dnabert2_splitB_seed42/slurm-%j.out
#SBATCH --error=results/dnabert2_splitB_seed42/slurm-%j.err

# ============================================================
# ChromaGuide: dnabert2 | Split B | Seed 42
...
mkdir -p results/dnabert2_splitB_seed42
...
if [ -f results/dnabert2_splitB_seed42/results.json ]; then
    echo ""
    echo "=== RESULTS ==="
    python -c "
import json
with open('results/dnabert2_splitB_seed42/results.json') as f:
    r = json.load(f)
