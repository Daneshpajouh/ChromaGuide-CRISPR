#!/usr/bin/env python3
"""Submit all 45 ChromaGuide experiments to Fir (H100-80GB, 4 GPUs/node)."""
import os
import subprocess
import sys
...
# Fir needs explicit profile source for modules
MODULE_LOADS = """source /cvmfs/soft.computecanada.ca/config/profile/bash.sh
module purge
...
echo "===== ChromaGuide v2: {backbone} | Split {split} | Seed {seed} | Fir (H100-80GB) ====="
...
def main():
    ...
    for backbone in BACKBONE_CONFIG:
        ...
        for split in SPLITS:
            for seed in SEEDS:
                ...
                script_path = os.path.join(SLURM_SCRIPTS_DIR, f"{job_name}-fir.sh")
    ...
    print(f"\n{'='*60}\nFIR SUBMISSION: {len(submitted)}/45")
    if errors:
    ...
    if submitted:
        with open(os.path.join(PROJECT, "fir_job_ids.txt"), "w") as f:
