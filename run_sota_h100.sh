...
echo "=== CRISPRO-MAMBA-X SOTA Training on H100 ==="
echo "Date: $(date)"
echo "Host: $(hostname)"
...
# Clean checkpoints to ensure fresh init
echo "=== Clearing old checkpoints for fresh training ==="
rm -f checkpoints/*.pth

# Run SOTA Training (Full Dataset, 100 epochs)
echo "=== Starting SOTA Training (Full 238k Dataset) ==="
python -m src.train --production 2>&1

if [ $? -eq 0 ]; then
    echo "✅ SOTA TRAINING SUCCESSFUL"
else
    echo "❌ SOTA TRAINING FAILED (Code: $?)"
fi
