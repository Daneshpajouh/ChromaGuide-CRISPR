#!/bin/bash
#SBATCH --job-name=pub_off_uncert_cpu
#SBATCH --account=def-kwiese
#SBATCH --mem=48G
#SBATCH --time=8:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=slurm_logs/public_off_target_uncertainty_cpu_%j.out
#SBATCH --error=slurm_logs/public_off_target_uncertainty_cpu_%j.err
...
LR="${LR:-0.0003}"
OUTPUT_JSON="${OUTPUT_JSON:-results/public_benchmarks/public_off_target_uncertainty_change_seq_cpu.json}"
MODEL_OUT="${MODEL_OUT:-results/public_benchmarks/public_off_target_uncertainty_change_seq_cpu.pt}"
DRY_RUN="${DRY_RUN:-0}"
