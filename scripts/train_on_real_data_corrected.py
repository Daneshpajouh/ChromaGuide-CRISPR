...
class BetaRegressionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = torch.clamp(mu, 1e-4, 1-1e-4)
        phi = self.softplus(out[:, 1:2]) + 2.0
...
def main():
    ...
    # Differential Learning Rates - MORE CONSERVATIVE FOR STABILITY
    ...
    for epoch in range(epochs):
        ...
        # Handle Constant Predictions for Spearman
        val_preds = np.array(val_preds)
        if np.std(val_preds) < 1e-9:
             val_preds += np.random.normal(0, 1e-8, size=val_preds.shape)

        val_rho, _ = spearmanr(val_preds, val_labels)
        print(f"Epoch {epoch+1} | Loss: {total_loss/(len(train_df)/batch_size):.4f} | Val Rho: {val_rho:.4f} | LR: {optimizer.param_groups[0]['lr']:.8f}")
