...
class LeakageControlledSplitter:
    ...
    def create_split_a_gene_held_out(self, datasets: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """Split A: Gene-held-out. 80% genes for training, 20% for testing."""
        logger.info("Creating Split A: Gene-held-out...")
        ...
        train_final = pd.concat(train_dfs).drop_duplicates(subset=['sequence'])
        test_final = pd.concat(test_dfs).drop_duplicates(subset=['sequence'])

        logger.info(f"  Split A: {len(train_final)} train samples ({len(train_genes)} genes), {len(test_final)} test samples ({len(test_genes)} genes)")
