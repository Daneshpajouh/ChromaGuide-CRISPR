...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_6_2.png}
    \caption[State Space Model (SSM) Equation Visualized]{Block diagram of the continuous-time State Space Model. The input $x(t)$ drives the state evolution $h'(t)$ via matrix $B$, while the state loops back via matrix $A$. The output $y(t)$ is projected from the state via matrix $C$. Discretization (ZOH) converts this to a recurrent sequence model.}
...
\begin{figure}[h!]
    ...
    \caption[Discretization Visualization]{Visualizing the Zero-Order Hold (ZOH) discretization process. The continuous varying signal (gray curve) is approximated by discrete steps (blue bars) of width $\Delta$, enabling digital computation of the continuous ODE.}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_6_1.png}
    \caption[The "Selective Scan" Gate]{Conceptual visualization of the Mamba Selective Scan mechanism. An input-dependent "Gate" (iris) dynamically adjusts the step size $\Delta$, effectively opening to store important information (Gold particles) into memory or closing to ignore noise (Gray particles).}
    \label{fig:selective_scan}
\end{figure}
...
\input{tables/tab_mamba_complexity}

\section{DNA Embedding and Input Processing}

\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_6_4.png}
    \caption[Context Window Scale Comparison]{Scale comparison of genomic context windows. Standard Transformers (Left) process only ~4kb (30cm ruler), while Mamba (Right) processes 1.2 Million base pairs (infinite tape), capturing long-range enhancer-promoter interactions missed by short-context models.}
    \label{fig:context_scale}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_6_3.png}
    \caption[Hardware Efficiency: Quadratic vs Linear]{Computational complexity comparison. (Left) Transformer's quadratic memory scaling ($O(N^2)$) leads to memory overflow on long sequences. (Right) Mamba's linear scaling ($O(N)$) maintains constant low memory footprint, enabling processing of megabase-scale inputs on single GPUs.}
...
\input{tables/tab_architecture_comparison}
...
\input{tables/tab_hyperparameters}
