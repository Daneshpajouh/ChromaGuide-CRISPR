...
\input{tables/tab_predicate_devices}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_12_4.png}
    \caption[Bedside Clinical Device]{Concept render of the CRISPRO-MAMBA-X beside device. The tablet-based interface allows clinicians to input patient data at the point of care, rapidly receiving guide recommendations and safety estimates via the secure hospital cloud.}
    \label{fig:bedside_device}
\end{figure}
...
\subsection{Statistical Robustness and Validity of Claims}

The claims made in this dissertation are supported by rigorous statistical validation, ensuring that the reported improvements are reliable and reproducible.

\subsubsection{Statistical Significance of Performance Gains}
As detailed in Chapter 9, the performance superiority of CRISPRO-MAMBA-X over Transformer baselines is statistically significant ($p < 10^{-47}$, Wilcoxon Signed-Rank Test). With a sample size of $N=20,000$, our analysis achieves $>99\%$ statistical power to detect even small effect sizes ($d > 0.1$). This confirms that the observed gains are not due to random chance or dataset idiosyncrasies.

\subsubsection{False Discovery Rate Control}
In our off-target analysis (Chapter 5), we applied the Benjamini-Hochberg procedure to control the False Discovery Rate (FDR) at $\alpha=0.05$. This ensures that fewer than 5\% of our predicted off-target sites are false positives, a critical safety threshold for clinical applications.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_12_3.png}
    \caption[Multi-Species Generalization]{Roadmap for multi-species expansion. The central hub represents the core Mamba model, with spokes extending to adapted models for Mouse, Zebrafish, Drosophila, and Plants. Transfer learning enables rapid adaptation to new species.}
    \label{fig:multi_species_wheel}
\end{figure}
...
\begin{enumerate}
    \item Implement thermodynamic binding energy calculation ($\Delta G_{bind}$) as interpretable layer
...
\input{tables/tab_final_summary}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_12_2.png}
    \caption[Future Vision: The Cure]{Artistic rendering of a healthy child leaving the clinic, representing the ultimate goal of this work. The background subtly incorporates the double helix and Mamba architecture curves, symbolizing the technology that enabled their cure.}
...
\bibitem{CASGEVY2023} CASGEVY (ex-vivo CTX001). (2023). FDA approval for sickle cell disease. Regulatory approval documentation.
