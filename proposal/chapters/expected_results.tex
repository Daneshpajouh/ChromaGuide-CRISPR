...
\begin{sloppypar}
\begin{itemize}
\item \textbf{Primary ranking performance:} On Split~A (gene-held-out), achieve at least $\Delta\rho\geq 0.03$ improvement over the ChromeCRISPR baseline when both are evaluated on the \emph{same} leakage-controlled splits. (ChromeCRISPR's reported $\rho=0.876$ was obtained under a random 85/15 split; we will re-evaluate the baseline under Split~A for like-for-like comparison.) See~\citep{daneshpajouh2025chromecrispr}. \textbf{Rationale:} This target derives from empirical evidence that chromatin accessibility features contribute 3--5\% additional predictive power beyond sequence-only models in comparable CRISPR prediction tasks~\citep{kim2020deepcrispr}, and recent multi-modal fusion approaches demonstrate similar gains when integrating epigenomic signals~\citep{belghazi2018,cheng2020}. 9
The \textbf{SOTA comparison:} This target is designed to \emph{surpass all current state-of-the-art models} on the leakage-controlled benchmark. The $\Delta\rho \geq 0.03$ represents a \emph{conservative minimum bound}---we anticipate achieving higher performance given the synergistic integration of chromatin context with sequence features, but commit to this floor to ensure robust, reproducible advancement over existing methods. The resulting 9
($\rho \geq 0.906$, vs.\ ChromeCRISPR's $\rho=0.876$) that is statistically distinguishable with adequate power ($n \geq 1000$, $\alpha=0.001$).
