...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_1.png}
    \caption[ATAC-seq Mechanism]{The Physics of Accessibility (ATAC-seq): (Left) Closed chromatin (red spheres) blocks enzyme access, resulting in a flat signal. (Right) Open chromatin allows Tn5 enzyme (green) to clamp onto DNA, cutting it and generating a signal peak "spike". This visualizes the cause-and-effect between physical access and data signals.}
    \label{fig:atac_mechanism}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_2.png}
    \caption[H3K27ac Activation Switch]{H3K27ac as a "Molecular Switch": (Top) The H3K27ac mark (glowing green tip) acts as a high-visibility flag or antenna on the histone tail. (Bottom) This "Green Light" signal specifically recruits reader proteins (blue) that start the gene expression engine. This explains why H3K27ac is the strongest predictor of active enhancers to CRISPR-Cas9.}
    \label{fig:chipseq_process}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_3.png}
    \caption[Hi-C Genomic Bridge]{Hi-C as a "Genomic Bridge": (Left) Two genomic regions (Enhancer and Promoter) are separated by linear distance (1 million bases). (Right) Hi-C data reveals the 3D Loop that forms a physical "Handshake" or bridge, bringing them into direct contact (held by purple Cohesin ring). This explains how distant regions can regulate local CRISPR efficiency.}
    \label{fig:hic_matrix}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{equation}
    \resizebox{0.9\textwidth}{!}{$\text{ContactProfile}(p) = [C(p, p-100), C(p, p-50), C(p, p-10), \ldots, C(p, p+10), C(p, p+50), C(p, p+100)]$}
    \end{equation}
...
\begin{equation}
\text{Efficiency reduction from nucleosome-free to nucleosome-occupied:} \\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_4.png}
    \caption[DNA Methylation Roadblock]{DNA Methylation as a "Physical Roadblock": (Left) The Methyl group (red spike) physically blocks the DNA road. (Right) Incoming proteins (Transcription Factors) bounce off this spike, unable to land. This visualizes the "Do Not Enter" mechanism of gene silencing.}
    \label{fig:methylation_mechanism}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption[CRISPRO-MAMBA-X Dual-Stream Architecture]{The "Dual-Stream" Architecture (CRISPRO-MAMBA-X): This diagram visualizes the information flow. (Left) The Mamba Stream uses "Long Arcs" to model global, long-range sequence context. (Right) The Epigenomic Stream uses "Local Pyramids" (CNNs) to detect specific chromatin motifs. (Center) The streams fuse to form a unified, variance-aware prediction.}
    \label{fig:architecture}
\end{figure}

\subsection{4.2.3 Data Processing and Integration}
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_6.png}
    \caption[Multimodal Data Tensor]{Visualization of the Multimodal Input Tensor: A 3D tensor ($L \times N \times C$) aligning the DNA sequence with 5 parallel epigenomic tracks (ATAC, H3K27ac, etc.), ready for "Two-Eyed" processing.}
...
The Mamba state space model (Chapter 6) enables integration of all epigenomic modalities across long genomic context (1.2 Mbp). specifically, we employ the \textbf{BiMamba2} architecture \cite{Dao2024}, utilizing the Mamba-2 State Space Duality (SSD) kernel for $O(L)$ efficiency with bidirectional scanning to capture upstream and downstream context simultaneously.
...
\section{Optimization Objective: Hybrid Loss Function}

To directly optimize for ranking performance while maintaining calibration, we introduce a novel \textbf{Hybrid Loss Function} that combines regression accuracy, ranking capability, and biological constraints:

\begin{equation}
\mathcal{L}_{\text{Hybrid}} = \lambda_{\text{MSE}} \mathcal{L}_{\text{MSE}} + \lambda_{\text{Spearman}} \mathcal{L}_{\text{Spearman}} + \lambda_{\text{RC}} \mathcal{L}_{\text{RC}}
\end{equation}

where:
\begin{itemize}
    \item $\mathcal{L}_{\text{MSE}}$ is Smooth L1 Loss (Huber Loss) measuring absolute error. $\lambda_{\text{MSE}}=0.3$.
    \item $\mathcal{L}_{\text{Spearman}}$ is a differentiable approximation of $1 - \text{Spearman Correlation}$. This term directly maximizes the ranking metric used for evaluation. $\lambda_{\text{Spearman}}=0.6$.
    \item $\mathcal{L}_{\text{RC}}$ is the Reverse-Complement Consistency loss, penalizing divergence between forward and reverse strand predictions. $\lambda_{\text{RC}}=0.1$.
\end{itemize}

This multi-objective loss ensures the model prioritizes correct ranking (highest biological utility) over minimizing negligible scale errors.

\section{Computational Learning Modules}

To fully leverage these epigenomic signals, the CRISPRO-MAMBA-X framework incorporates advanced computational modules for graph learning, contrastive alignment, and uncertainty quantification.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_7.png}
    \caption[Epigenomic Graph Neural Network]{The Epigenomic Graph: Modeling 3D chromatin interactions as a graph. Nodes represent genomic bins (nucleosomes), and edges represent Hi-C contacts. This allows the model to "hop" across the graph to aggregate information from distant enhancers.}
    \label{fig:epigenomic_graph}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_8.png}
    \caption[Contrastive Learning for Alignment]{Contrastive Learning for Signal Alignment: Using a "Push/Pull" loss function (InfoNCE) to align the Sequence representation (Mamba) with the Epigenomic representation (CNN). Positive pairs (matched locations) are pulled together; negative pairs are pushed apart.}
    \label{fig:contrastive_learning}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_9.png}
    \caption[Distribution Shift Robustness]{Quantifying Distribution Shift: Visualizing the domain shift problem. (Red) The training distribution (e.g., K562 cells) and (Blue) The testing distribution (e.g., Clinical T-cells). The model learns to be robust where these histograms overlap.}
    \label{fig:distribution_shift}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/fig_4_10.png}
    \caption[Conformal Prediction Funnel]{Conformal Prediction for Reliability: The "Confidence Funnel". The model outputs not just a prediction (dot), but a validity interval (shaded region) that is guaranteed to contain the true efficiency 90\% of the time, essential for clinical safety.}
