...
% FIXED: Removed problematic Greek letters from verbatim blocks
% ======================================================================
...
├─ On-Target Efficiency: [e-hat - q_alpha, e-hat + q_alpha]
...
└─ Off-Target Risk: [p-hat - q'_alpha, p-hat + q'_alpha]
...
\hline
On-Target Prediction & 3 & Sequence+Epigenomics \(\to\) Efficiency & \(\hat{e} \in [0,1]\) \\
\hline
...
\State Retain sites with <= 3 mismatches (configurable threshold)
...
\begin{equation}
\mathbf{u}_k = [\text{RNA-FM embedding}_k; \text{ATAC}_k; \text{H3K27ac}_k; \text{Nucleosome}_k; \text{Methylation}_k; \text{Hi-C}_k] \in \mathbb{R}^{512+6}
\end{equation}
...
\begin{equation}
\text{LayerNorm}(\mathbf{h}_{\text{target}}) \to \text{ReLU Dense}(512 \to 256) \to \text{ReLU Dense}(256 \to 128) \to \text{Dense}(128 \to 1)
\end{equation}
...
\begin{equation}
\text{LayerNorm}(\mathbf{h}_{\text{offtarget}} + \text{context embedding}) \to \text{ReLU Dense}(512 \to 256) \to \text{ReLU Dense}(256 \to 64) \to \text{Dense}(64 \to 1)
\end{equation}
...
\hline
Total GPU memory & \(32 \times 3.7 = 118\) GB & Distribute across 3x A100 GPUs \\
\hline
Training time (per epoch) & 1.5 hours & 100K samples, 32 batch size \\
\hline
Total training time & \(30 \times 1.5\) h = 45 hours & Including validation, ~2 days \\
\hline
GPU recommendation & 3x A100 (80 GB each) & Or 6x A6000 (48 GB) \\
\hline
\end{tabular}
\end{table}
...
For calibration set of 10,000 samples with efficiency and off-target nonconformities:
...
Day 5: Experimental Optimization
...
Continuous: CRISPRO Learning and Improvement
...
\section{Maintenance, Monitoring, and Continuous Improvement}
