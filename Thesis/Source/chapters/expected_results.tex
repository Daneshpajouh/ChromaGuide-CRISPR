...
\begin{sloppypar}
\begin{itemize}
\item \textbf{Primary ranking performance:} On Split~A (gene-held-out), achieve at least $\Delta\rho\geq 0.035$ improvement over the ChromeCRISPR baseline when both are evaluated on the \emph{same} leakage-controlled splits. (ChromeCRISPR's reported $\rho=0.876$ was obtained under a random 85/15 split; we will re-evaluate the baseline under Split~A for like-for-like comparison.) See~\citep{daneshpajouh2025chromecrispr}. \textbf{Rationale:} This target derives from empirical evidence that chromatin accessibility features contribute 3--5\% additional predictive power beyond sequence-only models in comparable CRISPR prediction tasks~\citep{kim2020deepcrispr}, and recent multi-modal fusion approaches demonstrate similar gains when integrating epigenomic signals~\citep{kimata2025dnabertepi,xiang2021enhancing}.
The \textbf{SOTA comparison:} This target is designed to \emph{match or surpass current state-of-the-art models} on the leakage-controlled benchmark. The $\Delta\rho \geq 0.035$ represents a \emph{conservative minimum bound}---we anticipate achieving higher performance given the synergistic integration of chromatin context with sequence features, but adopt this floor as a working target to ensure robust, reproducible advancement over existing methods. \textbf{Baseline contingency:} If re-evaluated baselines (re-trained under our leakage-controlled protocol) achieve higher performance than originally reported---narrowing the gap available for improvement---we will (i) report the updated baseline performance transparently, (ii) adjust our improvement claims to reflect the revised gap, and (iii) emphasize ChromaGuide's complementary contributions (uncertainty quantification, interpretability, off-target integration) that existing models do not provide, regardless of the on-target performance margin.11

($\rho \geq 0.911$, vs.\ ChromeCRISPR's $\rho=0.876$) that is statistically distinguishable with adequate power ($n \geq 1000$, $\alpha=0.001$).
\item \textbf{Statistical significance:} All performance claims will be validated using paired statistical tests (Wilcoxon signed-rank test or paired $t$-test where appropriate) with significance threshold $p < 0.001$, ensuring improvements are not due to random variation. We will report effect sizes (Cohen's $d$) alongside $p$-values.
...
\begin{itemize}
\item \textbf{Multi-modal fusion with an explicit non-redundancy objective (MINE/CLUB).} We will develop and ablate a fusion mechanism that is expressive (e.g., gating/attention/MLP fusion) while explicitly discouraging redundant sequence--epigenomic representations~\citep{kimata2025dnabertepi,xiang2021enhancing}.
