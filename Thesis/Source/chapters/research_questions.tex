...
\textbf{Operationalization:} On a pre-registered, leakage-controlled gene-held-out test split (Split~A), ChromaGuide \emph{aims to surpass current state-of-the-art models}, improving Spearman correlation over the best existing baseline by at least $\Delta\rho = 0.035$ (e.g., $0.876\to 0.896$) while maintaining conformal prediction-interval coverage within $\pm 0.02$ of the target level (e.g., 0.90). (ChromeCRISPR's reported $\rho=0.876$ was obtained under a random 85/15 split; we will re-evaluate the baseline under Split~A for like-for-like comparison.)
